...
rf_df1 <- subset (df, select = c(cluster, hTopic_01:tTopic_08))
rf_df1$cluster <- as.factor (rf_df1$cluster)
...
train_rf_df1 <- rf_df1[tindex, ]
test_rf_df1 <- rf_df1[-tindex, ]

rf1 <- randomForest (cluster~., data = train_rf_df1, importance = TRUE)
print (rf1)

prediction1 <- predict (rf1, newdata = test_rf_df1, type = "class")

misclassification_error_rate <- sum(test_rf_df1$cluster != prediction1) / 
  nrow(test_rf_df1)*100
